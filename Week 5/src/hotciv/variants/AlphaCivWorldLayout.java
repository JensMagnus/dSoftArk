...
public class AlphaCivWorldLayout implements WorldLayout {
	...
	public HashMap<Position, UnitImpl> defineWorldUnits(Game game) {
		unitMap.put(new Position(2,0), new UnitImpl(GameConstants.ARCHER, Player.RED, game.getUnitList()));
		unitMap.put(new Position(3,2), new UnitImpl(GameConstants.LEGION, Player.BLUE, game.getUnitList()));
		unitMap.put(new Position(4,3), new UnitImpl(GameConstants.SETTLER, Player.RED, game.getUnitList()));
		return unitMap;
	}
}
